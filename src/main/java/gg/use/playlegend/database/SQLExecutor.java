...
public class SQLExecutor implements AutoCloseable {
  ...
  public void createTable(Table table) throws SQLException {
    SQLBuilder builder = new SQLBuilder().setStatement("CREATE TABLE IF NOT EXISTS " + table + "(" + table.getSchema() + ")");
    this.executeUpdate(builder.build());
  }

  public void dropTable(Table table) throws SQLException {
    SQLBuilder builder = new SQLBuilder().setStatement("DROP TABLE IF EXISTS " + table);
    this.executeUpdate(builder.build());
  }

  public boolean rowExists(Table table, String condition, Object... params) throws SQLException {
    SQLBuilder builder = new SQLBuilder().select(table, "1").where(condition);
